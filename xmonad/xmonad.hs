...
myKeys x = [ ((mod4Mask .|. shiftMask, xK_Return), windows W.swapMaster)
           ...
           , ((mod4Mask .|. mod1Mask, xK_h), sendMessage $ pullGroup L)
           , ((mod4Mask .|. mod1Mask, xK_l), sendMessage $ pullGroup R)
           , ((mod4Mask .|. mod1Mask, xK_k), sendMessage $ pullGroup U)
           , ((mod4Mask .|. mod1Mask, xK_j), sendMessage $ pullGroup D)
           , ((mod4Mask .|. mod1Mask, xK_m), withFocused (sendMessage . MergeAll))
           , ((mod4Mask .|. mod1Mask, xK_u), withFocused (sendMessage . UnMerge))
           , ((mod4Mask .|. mod1Mask, xK_period), onGroup W.focusUp')
           , ((mod4Mask .|. mod1Mask, xK_comma), onGroup W.focusDown')
           ...
           , ((controlMask .|. mod1Mask, xK_h), spawn "rofi -modi 'clipboard:greenclip print' -show clipboard -run-command '{cmd}'")

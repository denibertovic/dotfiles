...
import XMonad.Actions.Navigation2D (screenGo, screenSwap, windowToScreen)
...
import XMonad.StackSet (Screen(..), StackSet(..))
...
import XMonad.Util.Run (runInTerm, safeSpawn, safeSpawnProg, spawnPipe)
...
myPromptFont = "xft:Monospace-Bold:pixelsize=18"
...
myKeys x =
  let subKeys str ks = subtitle str : mkNamedKeymap x ks
      screenKeys = ["w", "e", "r"]
      dirKeys = ["j", "k", "h", "l"]
      arrowKeys = ["<D>", "<U>", "<L>", "<R>"]
      dirs = [D, U, L, R]
      ...
      zipM' m nm ks as f b =
        zipWith (\k d -> (m ++ k, addName nm $ f d b)) ks as
   in subKeys
        "My keys"
        ...
        , ( "M-S-q"
          , addName "Quit XMonad" $
            confirmPrompt hotPromptTheme "exit" $ io exitSuccess)
        ...
        , ( "M-S-<Backspace>"
          , addName "Killall windows on workspace" $
            confirmPrompt hotPromptTheme "kill all" killAll)
        , ("M-n", addName "Hide window ?????" $ withFocused hideWindow)
        , ( "M-o"
          , addName "Switch project" $ switchProjectPrompt warmPromptTheme)
        ...
        , ( "M-M1-m"
          , addName "Merge all into tabs" $ withFocused (sendMessage . MergeAll))
        , ( "M-M1-u"
          , addName "Unmerge all tabs" $ withFocused (sendMessage . UnMergeAll))
        ...
        , ("M-m", addName "Focus master" focusMaster)
           -- Scratchpads
        , ( "M-C-h"
          , addName "Launch HTOP" $ namedScratchpadAction scratchpads "htop")
        , ( "M-C-]"
          , addName "Launch Weechat" $
            namedScratchpadAction scratchpads "weechat")
        , ( "M-C-g"
          , addName "Launch Youtube Music" $
            namedScratchpadAction scratchpads "youtubeMusic")
        , ( "M-C-["
          , addName "Launch Signal" $ namedScratchpadAction scratchpads "signal")
        , ( "M-C-t"
          , addName "Launch Trello" $ namedScratchpadAction scratchpads "trello")
        , ( "M-C-p"
          , addName "Launch 1Password" $
            namedScratchpadAction scratchpads "1Password")
        , ( "M-C-o"
          , addName "Launch YubiOAuth" $
            namedScratchpadAction scratchpads "yubioath")
        ...
        , ( "M-S-<Print>"
          , addName "Interactive Print screen" $
            spawn "gnome-screenshot --interactive")
        , ("M-S-l", addName "Lock screen" $ spawn "xscreensaver-command -lock")
        , ( "C-M1-h"
          , addName "Launch clipboard manager" $
            spawn
              "rofi -modi 'clipboard:greenclip print' -show clipboard -run-command '{cmd}'")
        , ("M-\\", addName "Launch Google Chrome" $ spawn chrome)
        , ( "M-C-\\"
          , addName "Launch Google Chrome With Profile" $
            spawn
              "rofi -modi 'Chrome Profile':/home/deni/dotfiles/scripts/rofi-chrome-profile-launcher.sh -show 'Chrome Profile'")
        , ( "M-i"
          , addName "Launch Google Chrome Icognito" $ spawn chromeIncognito)
        , ("M-/", addName "Launch Firefox" $ spawn firefox)
        , ( "M-C-/"
          , addName "Launch Firefox With Profile" $
            spawn
              "rofi -modi 'Firefox Profile':/home/deni/dotfiles/scripts/rofi-firefox-profile-launcher.sh -show 'Firefox Profile'")
        ...
        , ("M-S-b", addName "Bring window" bringMenu)
           -- dunst
        , ( "M-S-<Delete>"
          , addName "Redisplay last notification" $ spawn "dunstctl history-pop")
        , ("M-<Delete>", addName "Close notification" $ spawn "dunstctl close")
        , ( "M-C-<Delete>"
          , addName "Close all notifications" $ spawn "dunstctl close-all")
           -- Monitors
        , ( "M-C-<F11>"
          , addName "Laptop monitor" $
            spawn "/home/deni/dotfiles/scripts/monitors_laptop.sh")
        , ( "M-C-<F12>"
          , addName "Work monitor" $
            spawn "/home/deni/dotfiles/scripts/monitors_work.sh")
        ] ^++^
      subKeys
        "Media Keys"
        [ ( "<XF86AudioMute>"
        ...
        , ( "<XF86MonBrightnessUp>"
          , addName "Increase brightness" $
            spawn "/home/deni/dotfiles/scripts/brightness.sh +10")
        , ( "<XF86MonBrightnessDown>"
          , addName "Decrease brightness" $
            spawn "/home/deni/dotfiles/scripts/brightness.sh -10")
        ]
...
getOffset :: X (Int, Int)
getOffset =
  withWindowSet $ \W.StackSet {current = W.Screen {screenDetail = SD {screenRect = Rectangle { rect_x = x
                                                                                             , rect_y = y
                                                                                             }}}} ->
...
boseConnectCmd :: X ()
boseConnectCmd = spawn "bluetoothctl connect 28:11:A5:D9:25:59"

boseDisconnectCmd :: X ()
boseDisconnectCmd = spawn "bluetoothctl disconnect 28:11:A5:D9:25:59"
...
mehCmd = do
  ...
  let y
        | yOffset /= 0 && length screens > 1 = 720
...
centered w =
  ...
  D.addArgs ["-fg", lightgray] >=> D.addArgs ["-bg", black]
...
myManageHook =
  ...
  manageSpecific <+>
  manageDocks <+>
  namedScratchpadManageHook scratchpads <+> fullscreenManageHook <+> manageSpawn
  where
    manageSpecific =
      composeOne
        ...
        , (appName =? "Msgcompose") <&&> (className =? "thunderbird-default") -?>
          doCenterFloat
        ...
        , className =? "Browsers" -?> doCenterFloat
    ...
    isDialog' =
      isInProperty "_NET_WM_WINDOW_TYPE(ATOM)" "_NET_WM_WINDOW_TYPE_DIALOG"
    isSplash =
      isInProperty "_NET_WM_WINDOW_TYPE(ATOM)" "_NET_WM_WINDOW_TYPE_SPLASH"
    isSplash' =
      isInProperty "_NET_WM_WINDOW_TYPE(ATOM)" "_NET_WM_WINDOW_TYPE_SPLASH"
    isUtility =
      isInProperty "_NET_WM_WINDOW_TYPE(ATOM)" "_NET_WM_WINDOW_TYPE_UTILITY"
...
scratchpads
 ...
 =
  ...
  , NS
      ...
      yubioathCommand
      isYubioath
      (customFloating $ W.RationalRect (1 / 3) (1 / 6) (2 / 6) (2 / 3))
...
myHandleEventHook =
  ...
  handleEventHook def <+>
  fullscreenEventHook <+>
  Hacks.trayerAboveXmobarEventHook <+> Hacks.trayerPaddingXmobarEventHook
...
myLogHook h
 ...
 = do
  ...
  let check ws
        | ws `elem` copies = pad . xmobarColor yellow red . wrap "*" " " $ ws
        | otherwise = pad ws
  fadeWindowsLogHook myFadeHook
  ewmhDesktopsLogHook
  dynamicLogWithPP
    xmobarPP
      {ppOutput = hPutStrLn h, ppTitle = xmobarColor "green" "" . shorten 30}
...
showKeybindings x =
  ...
  io $ do
    h <-
      spawnPipe "zenity --font=terminus --text-info --width=1500 --height=1000"
